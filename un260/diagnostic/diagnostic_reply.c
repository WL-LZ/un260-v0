...
{
    if (index >= 0x01 && index <= SENSOR_VOLTAGE_CH_NUM) {
        return (int)index - 1;
    }
    return -1;
}

static diagnostic_reply_result_t diagnostic_sensor_reply_handle(
    const uint8_t *buf,
    uint8_t len)
{
    uint8_t index;
    ...
    if (index == 0xFF && value == 0xFF) {
        return DIAGNOSTIC_REPLY_SENSOR_END;
    }
    ...
    const diagnostic_reply_hooks_t *hooks)
{
    ...
    if (hooks != NULL && hooks->on_calibration_changed != NULL) {
        hooks->on_calibration_changed();
    }
